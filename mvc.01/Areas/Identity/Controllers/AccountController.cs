...
namespace mvc01.Areas.Identity.Controllers
{
    [Authorize]
    [Area("Identity")]
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> SendCode(string returnUrl = null, bool rememberMe = false)
        {
            ...
            if (user == null)
            {
                // Direct access without proper 2FA flow - redirect to login
                return Redirect($"/login/?returnUrl={Uri.EscapeDataString(returnUrl ?? "/")}");
            }
            var userFactors = await _userManager.GetValidTwoFactorProvidersAsync(user);
            if (userFactors == null || !userFactors.Any())
            {
                // No 2FA providers configured - redirect to manage page
                return Redirect($"/Member/Index?area=Identity");
            }
